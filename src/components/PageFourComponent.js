...
import { call4 } from '../services/DataServices.js'
...
export default function PageFourComponent() {
    ...
    const getResult = async () => {
        if (numOne === "" && numTwo == "") {
            setResult("Whats Your Name?  When Did You Wake Up?")
        } else {
            const response = await call4(numOne, numTwo);
            setResult(response)
        }
    }
    ...
    return (
        <div class="bg-[#FAC886] py-[1px]">
            ...
            <div
                ...
                <p id="p3Text" class="lg:text-[100px] text-[20px]">{result === "" ? " Whats Your Name?  When Did You Wake Up?" : result}</p>
            </div>
