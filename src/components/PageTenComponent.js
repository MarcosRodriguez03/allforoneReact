...
import { call10 } from '../services/DataServices.js'
...
export default function PageTenComponent() {
    ...
    const getResult = async () => {
        if (name === "") {
            setResult("Enter Question for Advice")
        } else {
            const response = await call10(name);
            setResult(response)
        }
    ...
    return (
        ...
        <div className="bg-[#6B866C] py-[1px]">
            <div
                ...
                <div id="p2Text" className="lg:text-[100px] text-[20px]">{result === "" ? "Enter Question for Advice " : result}</div>
